package io.nozistance.sftpclient;
...
public final class SftpClient implements AutoCloseable {
    ...
    /**
     * Get list of all files including directories
     *
     * @param remoteDir Directory on remote from which files will be getting
     * @throws SftpException If there is any problem with getting list of files related to permissions etc
     */
    public Set<ChannelSftp.LsEntry> getFilesList(String remoteDir) throws SftpException {
        if (channel == null) {
            throw new IllegalArgumentException("Connection is not available");
        }
        channel.cd(remoteDir);
        return new LinkedHashSet<>(channel.ls("."));
    }
